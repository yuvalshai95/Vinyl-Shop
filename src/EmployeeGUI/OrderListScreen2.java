...
public class OrderListScreen2 {

	private JFrame orderListFrame2;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					OrderListScreen2 window = new OrderListScreen2("TEST");
					window.orderListFrame2.setVisible(true);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	 ...
	 */
	public OrderListScreen2(String employeeEmail) {
	...
	private void initialize() {
		orderListFrame2 = new JFrame("Yuval Shai & Adi Hemo");
		orderListFrame2.setBounds(100, 100, 744, 600);
		orderListFrame2.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		orderListFrame2.getContentPane().setLayout(null);
		//Center The Frame to Monitor
		Dimension dim = Toolkit.getDefaultToolkit().getScreenSize();
		orderListFrame2.setLocation(dim.width/2-orderListFrame2.getSize().width/2, dim.height/2-orderListFrame2.getSize().height/2);
		
		// Set Icon to the frame
		ImageIcon icon = new ImageIcon("record.png");
		orderListFrame2.setIconImage(icon.getImage());
		...
		orderListLabel.setBounds(18, 256, 700, 43);
		orderListFrame2.getContentPane().add(orderListLabel);
		...
		orderListLabel_1.setBounds(10, 13, 708, 43);
		orderListFrame2.getContentPane().add(orderListLabel_1);
		
		JScrollPane orderDescriptionScrollPane = new JScrollPane();
		orderDescriptionScrollPane.setBounds(178, 312, 377, 160);
		orderListFrame2.getContentPane().add(orderDescriptionScrollPane);
		...
		//Image back Icon
		Image back = new ImageIcon(this.getClass().getResource("/backToMenu.png")).getImage();
		returnToMenuBTN.setIcon(new ImageIcon(back));
		returnToMenuBTN.setBackground(new Color(220, 220, 220));
		returnToMenuBTN.setFont(new Font("Tahoma", Font.BOLD, 11));
		returnToMenuBTN.setBounds(292, 500, 164, 50);
		orderListFrame2.getContentPane().add(returnToMenuBTN);
		
		JScrollPane orderListScrollPane = new JScrollPane();
		orderListScrollPane.setBounds(10, 56, 708, 171);
		orderListFrame2.getContentPane().add(orderListScrollPane);
		...
		orderListTable.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				index = orderListTable.getSelectedRow();
				int selectedRecordId = (int) model.getValueAt(index, 0);
				
				order = DB_Main.db_RecordShop.getOrderByID(selectedRecordId);
				
				// Set all records from order to the records list from sql
		...
		orderListFrame2.getContentPane().add(packageLabel1);
		...
		orderListFrame2.getContentPane().add(packageLabel2);
